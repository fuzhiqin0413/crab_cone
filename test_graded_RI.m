...
function [X, T, new_delta_t] = ray_interpolation(type, X0, T0, delta_t, vol_coords, vol_inds, lens_volume)
    ...
    elseif type == 3
        ...
        %%% Should include include Ci to change delta T? DT_dt not function of T...
        %%% %%% Note, should modify as in anisotropic code...
            %%% Test difference.
        
        error('Modify x off set as in anisotropic code')    
...
function [dT_dt, n_of_X, dn_partial_dX] = numerical_dT_dt(X, vol_coords, vol_inds, lens_volume)
        ...
        for i = 1:size(sampling_coords,1)
            ...
            P_of_X(i,:) = calculatetricubicbasis(sampling_coords(i,1), sampling_coords(i,2), sampling_coords(i,3))';
        end
        ...
        [p_of_X, dp_partial_dX] = calculatetricubicbasis(X(1), X(2), X(3)); %20x1 and 20x3
